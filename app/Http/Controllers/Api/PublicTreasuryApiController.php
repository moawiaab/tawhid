...
class PublicTreasuryApiController extends Controller
{
    public function index()
    {
        ...
        return OpenDaysResources::collection(OpenDay::where('account_id', auth()->user()->account_id)->advancedFilter()->paginate(request('limit', 10)));
    }
    ...
    {
        $publicTreasury = PublicTreasury::where('account_id', auth()->user()->account_id)->first();
    ...
    public function done(OpenDay $item)
    {
        ...
        $pub = PublicTreasury::where('account_id', auth()->user()->account_id)->first();
