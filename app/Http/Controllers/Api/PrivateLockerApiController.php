...
class PrivateLockerApiController extends Controller
{
    public function index()
    {
        ...
        return  PrivateLockerResource::collection(PrivateLocker::where('account_id', auth()->user()->account_id)->advancedFilter()->paginate(request('limit', 10)));
    }

    public function store(StorePrivateLockerRequest $request)
    {
        abort_if(Gate::denies('safe_create'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        $safe->sh_id = auth()->user()->account_id;
        $safe->save();
