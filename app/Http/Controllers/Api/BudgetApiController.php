...
class BudgetApiController extends Controller
{
        ...
        return response([
            'meta' => [
                'budgets' => BudgetName::where('status', 1)->whereNotIn(
                    'id',
                    Budget::where('account_id', auth()->user()->account_id)->where('stage_id', $stage->id)
                        ->pluck('budget_id')
                )->where(function ($q) {
                    $q->where('type', 1)->orWhere('account_id', auth()->user()->account_id);
                })->get(['name', 'id']),
    ...
    private function createStage(){
        $stage = Stage::whereYear('start_date', date('Y'))->where('account_id', auth()->user()->account_id)->first();
        if (!$stage) {
            $stage = new Stage();
            $stage->name = " السنة المالية للعام " . date("Y");
            $stage->status = 1;
